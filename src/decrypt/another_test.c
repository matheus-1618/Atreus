...
int main(int argc, char *argv[]) {
    ...
    //if(!CryptAcquireContext(&hProv, NULL, NULL, dwProvType, dwFlags)){
    if (!CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
        printf("Error acquiring cryptographic context3\n");
        return 1;
    }
    ...
    if (content != NULL) {
        ...
        printf("Size of aes:%d",aesKeySize);
	 HCRYPTKEY hAesKey = 0;
    
        if (CryptImportKey(hProv, content_after_pattern,content_size , hKey, CRYPT_EXPORTABLE, &hAesKey)) {
            printf("AES key import successful!\n");
        } else {
            printf("AES key import failed. Error: %d\n", GetLastError());
        }
